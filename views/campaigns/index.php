	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col">
				...
				<h2><?php _e('Campaigns', gplat_textdomain) ?></h2>
			</div>
		</div>
		<div class="row">
			<div class="col">
			...
			<div class="col">
				<div class="card bg-light mb-3">
					<div class="card-body text-center">
						...
						<h5 class="card-title"><?php echo gplat_commons::numberize($gplat_agencies_count) . ' ' . _n('Campaign available', 'Campaigns available', $gplat_agencies_count, gplat_textdomain) ?></h5>
					</div>
				</div>
			</div>
		</div>
...
<script>
	jQuery(document).ready(function($) {
		$('#r_table').DataTable({
			ajax: "<?php echo gplat_dir_url . 'services/campaigns-service.php?service=get_campaigns' ?>",
			...
			columns: [
				...
				{
					title: "<?php _e('Campaign Name', gplat_textdomain) ?>", 
					data: null,
					...
					fnCreatedCell: function (nTd, sData, oData, iRow, iCol) {
						if(oData.campaign_type == 'SMS') {
							$(nTd).html('<a href="<?php echo admin_url('admin.php?page=gplat-admin-sms-campaign-detail&id=') ?>' + oData.id + '">' + oData.campaign_name + '</a>');
						} else if(oData.campaign_type == 'DEM') {
							$(nTd).html('<a href="<?php echo admin_url('admin.php?page=gplat-admin-dem-campaign-detail&id=') ?>' + oData.id + '">' + oData.campaign_name + '</a>');
						}
        			}
				},
				{
					title: "<?php _e('Agency', gplat_textdomain) ?>", 
					data: null,
					className: "small", 
					width: "150px", 
					type: "string",
					fnCreatedCell: function (nTd, sData, oData, iRow, iCol) {
						$(nTd).html('<a href="<?php echo admin_url('admin.php?page=gplat-admin-agency-detail&id=') ?>' + oData.agency_id + '">' + oData.agency_name + '</a>');
        			}
				},
				...
				{ title: "<?php _e('Created By', gplat_textdomain) ?>", data: "owner", width: "150px", className: "small", type: "string" },
			], 
		});
